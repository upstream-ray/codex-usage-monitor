...
- 系统托盘图标以百分比徽标显示已启用模型的用量
- 左键单击托盘图标可显示或隐藏任务栏组件
- 右键菜单支持刷新、模型选择、用量行、配额提醒、更新频率、语言、开机启动、组件显示和软件更新等设置
...
Antigravity 也是可选功能。若要显示其用量，请安装并登录 Google Antigravity，然后在右键菜单的**模型**中启用 **Antigravity**。
...
- 右键单击任务栏组件或托盘图标，可设置刷新、显示模型、用量行、配额提醒、更新频率、开机启动、重置位置、语言、软件更新和退出
- 左键单击托盘图标可显示或隐藏任务栏组件
- 如需登录 Windows 后自动运行，请在右键菜单中启用“开机启动”

### 模型

通过右键菜单中的**模型**选择组件要显示的内容：

- **Codex** 默认启用
- **Claude Code** 可与 Codex 同时显示，也可单独显示
- **Antigravity** 可与其他提供商同时显示，也可作为独立模型列单独显示

显示多个模型时，每个模型都有自己的用量条和对应的文字颜色。Antigravity 会优先使用 Google 的 Gemini 配额摘要，必要时回退到模型配额数据。

### 系统托盘图标

托盘图标以百分比徽标显示当前 5 小时用量。

启用多个提供商时，应用会为每个提供商显示一个托盘图标；只启用一个模型时，则只显示一个图标。

Claude Code 托盘图标使用与 Claude 用量条一致的暖色；Codex 图标采用黑白徽标样式；Antigravity 图标采用蓝色徽标样式。

将鼠标悬停在托盘图标上，会显示该模型的用量值和准确的本地重置时间。如果隐藏了 5 小时用量行，托盘徽标会改为显示每周用量百分比。
